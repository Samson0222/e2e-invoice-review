...
const steps = [
  ...
  ['2', 'Run the pipeline', 'Classify, extract, validate VAT and totals, then suggest a GL account.'],
  ['3', 'Inspect the result', 'Review the status, any issues found, and the GL suggestion.'],
]

export function WelcomePortal({ onStart, onViewHistory }: WelcomePortalProps) {
  return (
      ...
      <Card className="w-full">
        <CardHeader className="border-b border-zinc-100 p-8 sm:p-10">
          ...
          <p className="max-w-xl pt-1 text-base leading-7 text-zinc-600">
            Upload an invoice or receipt and run the review pipeline before it enters bookkeeping.
          </p>
